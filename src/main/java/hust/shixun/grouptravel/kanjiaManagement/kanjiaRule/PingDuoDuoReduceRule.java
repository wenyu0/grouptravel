package hust.shixun.grouptravel.kanjiaManagement.kanjiaRule;
...
public class PingDuoDuoReduceRule implements ReduceRule {
    ...
    public List<BigDecimal> getReduceList(BigDecimal totalReduce, int totalReduceTimes) {
        ...
        BigDecimal firstNReduceTotal = BigDecimal.valueOf(totalReduce.doubleValue() * percentOfFirstNReduce / 100).setScale(2, RoundingMode.HALF_EVEN);
        List<BigDecimal> ret = sfReduceRule.getReduceList(firstNReduceTotal, firstNReduce);
        BigDecimal leftReduceTotal = totalReduce.subtract(firstNReduceTotal).setScale(2, RoundingMode.HALF_EVEN);
    ...
    public static void main(String[] args) {
        PingDuoDuoReduceRule rule = new PingDuoDuoReduceRule(40, 5);
        List<BigDecimal> list = rule.getReduceList(new BigDecimal("100.21"), 8);
